    ...
    def calculate_report_emissions(self, request):
        ...
        for space_type in [
            ...
            if total_area > 0:
                ...
                water_emission = (
                    water_consumption * water_factor * proportion
                )  # Multiply by `proportion`
                print(
        ...
        for mode, distance in travel.items():
            ...
            travel_emission = (
                float(distance) * travel_factor * proportion
            )  # Multiply by `proportion`
            print(
        ...
        for waste_type, amount in waste.items():
            ...
            waste_emission = (
                float(amount) * waste_factor * proportion
            )  # Multiply by `proportion`
            print(
                f"🗑️ Calculating waste emissions: {amount}kg × {waste_factor} × {proportion} = {waste_emission}"
            )
